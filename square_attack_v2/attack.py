...
def square_attack_l2(model, x: np.ndarray, y, corr_classified, eps, n_iters, p_init, metrics_path, targeted, loss_type):
    """ The L2 square attack """
    np.random.seed(0)
    
    log = utils.Logger('log/Flower.log')
    ...
    first_idx_to_fool = (margin_min > 0.0)
    last_idx_to_fool: np.ndarray
    ...
    for i_iter in range(n_iters):
        ...
        last_idx_to_fool = (margin_min > 0.0)
    ...
    print('ASR {:.2f}%'.format(100 * (first_idx_to_fool > last_idx_to_fool).sum() / first_idx_to_fool.sum() ))
